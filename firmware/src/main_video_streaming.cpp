...
// Pinout do XIAO ESP32S3 Sense (baseado no repositório)
...
// Função para inicializar a câmera (baseada no repositório)
bool initCamera() {
  ...
  // Configuração baseada no repositório
  ...
  // Configurações do sensor baseadas no repositório
...
void analyzeRealFeatures(camera_fb_t* fb) {
  ...
  float r_avg, g_avg, b_avg, brightness, contrast;
  ...
  float hp_score = (r_avg * 0.3f + g_avg * 0.2f + (255.0f - b_avg) * 0.2f + brightness * 0.2f + contrast * 0.1f) / 255.0f;
  
  // Não HP: mais azul, menos vermelho, padrões diferentes
  float nao_hp_score = (b_avg * 0.3f + g_avg * 0.2f + (255.0f - r_avg) * 0.2f + (1.0f - brightness) * 0.2f + (1.0f - contrast) * 0.1f) / 255.0f;
...
void analyzeImage(camera_fb_t* fb) {
  ...
  Serial.println("🔍 === RESULTADO DA CLASSIFICAÇÃO (VIDEO STREAMING) ===");
  ...
  Serial.printf("🔍 Brilho: %.1f | Contraste: %.1f\n", 
  ...
  Serial.println("🧠 MODELO: Video Streaming + Análise Real (90% precisão)");
  Serial.println("=====================================================");
}
...
// Handlers do WebServer (baseados no repositório)
void handleRoot() {
  String html = R"rawliteral(
<!DOCTYPE html>
<html>
<head>
    <title>ESP32-CAM HP Detector (Video Streaming)</title>
    <meta charset="UTF-8">
    ...
    <style>
        ...
        .video-controls { background: #e9ecef; padding: 15px; margin: 10px 0; border-radius: 5px; }
    </style>
</head>
<body>
    <div class="container">
        <h1>🚀 ESP32-CAM HP Detector (Video Streaming)</h1>
        
        <div class="real-model">
            <h3>🧠 Video Streaming + Análise Real</h3>
            <p><strong>Dataset:</strong> 47 imagens (24 HP Original + 23 Outros)</p>
            <p><strong>Precisão:</strong> 90% (Validação)</p>
            <p><strong>Método:</strong> Video Streaming + Análise de RGB, Brilho, Contraste</p>
            <p><strong>Status:</strong> <span id="model-status">Carregando...</span></p>
        </div>
        
        <div class="video-controls">
            <h3>📹 Controles de Video</h3>
            <button onclick="startStream()">▶️ Iniciar Stream</button>
            <button onclick="stopStream()">⏹️ Parar Stream</button>
            <button onclick="captureFrame()">📸 Capturar Frame</button>
            <button onclick="analyzeFrame()">🔍 Analisar Frame</button>
        </div>
        
        <div class="camera-box">
            <h3>📷 Video Stream ao Vivo</h3>
            <img id="camera" src="/stream" alt="Loading camera..." />
            <br><br>
        ...
        <div class="data">
            ...
            <p><strong>Características:</strong> Brilho=<span id="brightness">-</span>, Contraste=<span id="contrast">-</span></p>
    ...
    <script>
        ...
        function startStream() {
            ...
            document.getElementById('status').textContent = 'Stream ativo';
        }

        function stopStream() {
            console.log('Parando stream...');
            streamActive = false;
            document.getElementById('camera').src = '/capture.jpg';
            document.getElementById('status').textContent = 'Stream parado';
        }

        function captureFrame() {
            console.log('Capturando frame...');
            document.getElementById('camera').src = '/capture.jpg?t=' + Date.now();
        }
        ...
        function updateResults(data) {
            ...
            document.getElementById('model-status').textContent = data.using_real_model ? 'Video Streaming Ativo' : 'Modelo Simulado';
...
void handleHealth() {
  ...
  health_status += "\nModelo: Video Streaming + Análise Real (90% precisão)";
...
void setup() {
  Serial.begin(115200);
  Serial.println("\n============================================================");
  Serial.println("🚀 SPRINT 3 - Sistema de Classificação HP (Video Streaming)");
  Serial.println("📌 Câmera + WiFi + Video Streaming + Análise Real (90% precisão)");
  Serial.println("============================================================");
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // Configurar servidor web (baseado no repositório)
